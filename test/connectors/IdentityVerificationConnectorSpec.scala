      ...
      "when the response contains some evidence sources we don't recognise" - {
        ...
        "must return a list of evidence sources we do recognise" in {
          ...
          running(app) {
            ...
            val recognizedSources = result.collect {
              case evidence: IdentityVerificationEvidenceSource if !evidence.isInstanceOf[UnrecognisedSource] => evidence
            }

            recognizedSources must contain theSameElementsAs List(NtcService, P60Service, PayslipService)
          }
        }
      }
    }
